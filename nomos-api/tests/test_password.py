...
def test_password_strength_no_uppercase():
    ...
    assert any("Grossbuchstaben" in e for e in errors)
...
def test_password_strength_no_special_char():
    errors = validate_password_strength("NoSpecialChar12")
    assert any("Sonderzeichen" in e for e in errors)


def test_password_strength_no_lowercase():
    errors = validate_password_strength("NOLOWER123!")
    assert any("Kleinbuchstaben" in e for e in errors)
